...
public class PlayerController : MonoBehaviour {
    ...
    void keyboardControls()
    {
        ...
        float hitdist = 0.0f;
        // If the ray is parallel to the plane, Raycast will return false.
        //get previouse mouse input to know if the player is moving ther mouse or just want ing to trackthe same line
        //var prevMousePos = Input.mousePosition;
        //if (playerPlane.Raycast(ray, out hitdist) && (Input.mousePosition != prevMousePos))
        if (playerPlane.Raycast(ray, out hitdist))
        {
            // Get the point along the ray that hits the calculated distance.
            Vector3 targetPoint = ray.GetPoint(hitdist);
